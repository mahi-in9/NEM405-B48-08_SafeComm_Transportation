...
const getReportById = async (req, res) => {
    const report = await Reports_1.default.findById(req.params.id).populate("userId", "name email");
    if (!report)
        return res
            .status(404)
...
const updateReport = async (req, res) => {
    const report = await Reports_1.default.findByIdAndUpdate(req.params.id, req.body, {
        new: true,
    });
    if (!report)
        return res
